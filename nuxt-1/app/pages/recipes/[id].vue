        ...
        <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
          <li v-for="ingredient in data?.ingredients">
            <label class="flex gap-2 items-center">
              ...
              <div
                class="relative w-6 h-6 rounded-full border-2 border-[#f79f1a] flex items-center justify-center peer-checked:after:absolute peer-checked:after:w-4 peer-checked:after:h-4 peer-checked:after:bg-[#f79f1a] peer-checked:after:rounded-full"
              ></div>
      ...
      <div>
        ...
        <ul class="flex flex-col text-lg gap-4">
          <li v-for="(instruction, index) in data?.instructions" class="flex gap-2">
            <span
              class="flex items-center justify-center bg-[#f79f1a] w-7 h-7 rounded-full text-white text-sm"
            >
              {{ index + 1 }}
            </span>
